...
export const useSessionStore = create<SessionState>()(
  persist(
    (set, get) => ({
      ...
      bootstrap: async () => {
        if (!get().hasHydrated || get().isReady) return;
      ...
      signIn: async ({ username, password, rol, sede, jornada }) => {
        ...
        if (rol === "guarda") {
          ...
          if (estado.turno_activo && estado.turno) {
            set({ isReady: true, user: me.usuario, turno: estado.turno });
            return;
          }
          ...
          try {
            ...
            if (data?.turno) {
              set({ isReady: true, user: me.usuario, turno: data.turno });
              return;
            }
        ...
        set({ isReady: true, user: me.usuario, turno: null });
      },
